...
public class ResourceUtil {
    ...
    public static int getAppResInt(Application app,String key,int defaultValue) {
        ApplicationInfo info = null;
        try {
            info = app.getPackageManager().getApplicationInfo(app.getPackageName(),PackageManager.GET_META_DATA);
        } catch (PackageManager.NameNotFoundException e) {
            e.printStackTrace();
        }

        if (info == null) return defaultValue;
        int o = info.metaData.getInt(key,defaultValue);
